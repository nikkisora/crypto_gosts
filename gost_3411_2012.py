from utils import str_to_int, bit_length
...
def _S(m):
    res = 0
    for i in reversed(range(_block_size)):
        res = (res << 8) | _PI[(m>>(8*i))&0xFF]
    return res

def _P(m):
    res = 0
    for i in reversed(range(_block_size)):
        res = (res << 8) | ((m>>(8*_TAU[i]))&0xFF)
    return res

def _L(m):
    res = 0
    for i in reversed(range(8)):
        data_block = (m >> (64*i)) & 0xFFFFFFFFFFFFFFFF
        res_block = 0
        for j in reversed(range(_block_size)):
            if data_block & 0x1:
                res_block ^= _A[j]
            data_block >>= 1
        res <<= 64
        res |= (res_block) & 0xFFFFFFFFFFFFFFFF
    return res
...
def gost_hash(data, hash_size=512):
    if isinstance(data, str):
        data = str_to_int(data)
        
    # step 1
    hash = int(_block_size * ('01' if hash_size == 256 else '00'), 16) # initial vector
    n = 0
    epsilon = 0
    
    # step 2
    for i in range(0, bit_length(data)//512, 512):
        block = (data >> i) & int('FF'*_block_size, 16)
        hash = _g_N(hash, n, block)
        epsilon = (epsilon + block) & int('FF'*_block_size, 16)
        n += 512
        
    # step 3
    block = data >> n
    block_len = ceil(bit_length(block)/8)
    block |= 1 << (block_len * 8)
